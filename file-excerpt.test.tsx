...
describe("FileExcerpt", () => {
  ...
  it("renders an error when runDirectory is not a path prefix of src", async () => {
    const badMarkdown =
      '# Slide 1\n\n<FileExcerpt src="hello/hello.c" runDirectory="he" />\n';
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
      const url = typeof input === "string" ? input : input.toString();
      if (url.endsWith("/talks-static/sample-talk1.md")) {
        return { ok: true, text: () => Promise.resolve(badMarkdown) } as any;
      }
      if (url.endsWith("/talks-static/hello/hello.c")) {
        return { ok: true, text: () => Promise.resolve(helloC) } as any;
      }
      return { ok: false, status: 404, text: () => Promise.resolve("") } as any;
    }) as any;
    window.history.pushState({}, "", "/talks/sample-talk1/");
    const screen = await render(<App />);
    await expect
      .element(screen.getByText(/runDirectory.*not a path prefix/))
      .toBeVisible();
  });
